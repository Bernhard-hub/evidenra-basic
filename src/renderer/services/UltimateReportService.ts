...
import { ContinuationService } from '../../services/ContinuationService'; // 🚀 REVOLUTION: Completion guarantee
import { IntegrationHelper } from '../../../genesis-engine/src/IntegrationHelper.js'; // 🧬 GAPES Integration
...
export class UltimateReportService {
   ...
   * 🧬 GAPES-Enhanced: Optimized prompts through genetic evolution
   */
  static async generateUltimateReport(
    ...
    genesisAPIWrapper?: any // 🧬 GAPES Wrapper for prompt optimization
  ): Promise<{ success: boolean; content?: string; error?: string; wordCount?: number; cost?: number }> {

    try {
      // Step 1: Aggregate all data (BASIS + ENHANCED)
      console.log('🔍 ULTIMATE: Step 1 - Aggregating data...');
      onStatusUpdate?.('📊 Aggregating BASIS + ENHANCED data...');
      const ultimateData = await this.aggregateAllData(project);
      console.log('✅ ULTIMATE: Data aggregation complete', {
        documents: ultimateData.projectOverview.totalDocuments,
        categories: ultimateData.projectOverview.totalCategories,
        codings: ultimateData.projectOverview.totalCodings
      });

      // Step 2: Generate comprehensive scientific article in multiple sections for 8000+ words
      console.log('🔍 ULTIMATE: Step 2 - Starting section generation...');
      ...
      // 🚀 PHASE 2 OPTIMIZATION: Parallel section generation for -74% time reduction!
      // Generate ALL sections simultaneously instead of sequentially
      onStatusUpdate?.(`🚀 Generating all ${sections.length} sections in parallel...`);

      const sectionPromises = sections.map((section, i) => {
        console.log(`🔍 ULTIMATE: Preparing section ${i + 1}/${sections.length}: ${section.name}`);
        onStatusUpdate?.(`📝 Starting ${section.name} (${section.words} words target)...`);
        return this.generateArticleSection(
          section, ultimateData, apiSettings, language, i, sections.length, onStatusUpdate, genesisAPIWrapper
        );
      });

      console.log(`🔍 ULTIMATE: All ${sections.length} section promises created, waiting for completion...`);
      // Wait for all sections to complete (use allSettled to handle partial failures gracefully)
      const sectionResults = await Promise.allSettled(sectionPromises);
      console.log('✅ ULTIMATE: All sections completed/failed', {
        fulfilled: sectionResults.filter(r => r.status === 'fulfilled').length,
        rejected: sectionResults.filter(r => r.status === 'rejected').length
      });
      ...
      for (let i = 0; i < sectionResults.length; i++) {
        ...
        if (result.status === 'fulfilled' && result.value.success) {
          fullArticle += result.value.content + '\n\n---\n\n';
          totalWordCount += result.value.wordCount || 0;
          totalCost += result.value.cost || 0;
          onStatusUpdate?.(`✅ ${section.name} completed (${result.value.wordCount} words)`);
        } else {
          const errorMsg = result.status === 'rejected'
            ? result.reason?.message || 'Unknown error'
            : result.value.error || 'Generation failed';
          return { success: false, error: `Failed to generate ${section.name}: ${errorMsg}` };
        }
      }

      // Add final literature section
      onStatusUpdate?.('📚 Generating comprehensive literature references...');
      const referencesResult = await this.generateReferencesSection(ultimateData, apiSettings, language, onStatusUpdate, genesisAPIWrapper);
...
Schreibe JETZT einen vollständigen wissenschaftlichen Artikel mit UMFANGREICHER LITERATURINTEGRATION basierend auf den obigen REALEN DATEN. Verwende die bereitgestellten Autoren, Jahre und Textstellen für konkrete Zitationen. MINDESTENS 15-20 verschiedene Literaturverweise sind PFLICHT!`
   ...
   * 🧬 GAPES-Enhanced: Uses evolved prompts when available
   */
  private static async callAPIWithRetry(
    ...
    maxRetries: number = 5,
    onStatusUpdate?: (status: string) => void,
    genesisAPIWrapper?: any, // 🧬 GAPES Wrapper
    category?: string, // 🧬 GAPES category for prompt optimization
    operation?: string, // 🧬 GAPES operation name
    context?: any // 🧬 GAPES context
  ): Promise<{ success: boolean; content?: string; error?: string; wordCount?: number; cost?: number }> {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        // 🧬 GAPES-Enhanced API Call
        const result = genesisAPIWrapper && category && operation
   ...
   * 🧬 GAPES-Enhanced: Section-specific prompt optimization
   */
  private static async generateArticleSection(
    ...
    console.log(`🔍 SECTION START: ${section.name} (${section.words} words, ${sectionIndex + 1}/${totalSections})`);
...
- KEINE "[Fortsetzung folgt]" oder Unterbrechungen`
    ...
    // 🚀 REVOLUTION: Use ContinuationService to GUARANTEE complete ${section.words}-word sections!
    console.log(`🔄 SECTION: Calling ContinuationService for ${section.name}...`);
    onStatusUpdate?.(`🔄 Generating ${section.name} with completion guarantee (${section.words} words)...`);
    ...
    console.log(`✅ SECTION COMPLETE: ${section.name}`, {
      success: continuationResult.success,
      wordCount: continuationResult.wordCount,
      targetReached: continuationResult.targetReached
    });

    if (!continuationResult.success) {
      console.error(`❌ SECTION FAILED: ${section.name}`, continuationResult.error);
      return {
        success: false,
        error: continuationResult.error || 'Section generation failed'
      };
    }

    // 🔧 V7.5: Apply paragraph formatting like BASIS report
    const formattedContent = this.ensureParagraphFormatting(continuationResult.content || '');
   ...
   * 🧬 GAPES-Enhanced: Optimized literature compilation
   */
...
Schmidt, K. & Weber, L. (2022): Datenanalyse in der empirischen Sozialforschung. *Empirische Studien*, 28(2), 67-89.`
    ...
    // 🧬 GAPES-Enhanced call for references
    return this.callAPIWithRetry(
      ...
      'generate_references',
      { sectionType: 'references', documentCount: ultimateData.projectOverview.totalDocuments }
    );
  }
...
**WICHTIG:** Verwende MINDESTENS 15-20 Literaturverweise aus den obigen Primärquellen! Jede Hauptaussage muss mit konkreten Zitaten (Autor, Jahr, Seite) belegt werden.`;
  }
...
`;
  ...
  /**
   * 🔧 V7.5: Ensure proper paragraph formatting for HTML/Markdown rendering
   * (Same as ScientificArticleService for consistency)
   */
