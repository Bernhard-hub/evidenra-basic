...
function EVIDENRAApp() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const phases = [
        ...
        { name: '3-Persona Analyse aus Dokumenten', progress: 40, duration: 2000 },
  ...
  // Generate 3 personas from document content using AI
  async function generateDocumentPersonas(documents) {
    const combinedText = documents.map(doc => doc.content || doc.text || '').join('\n\n').substring(0, 10000);

    try {
      const apiKey = localStorage.getItem('claude_api_key') || localStorage.getItem('openai_api_key');
      const provider = localStorage.getItem('claude_api_key') ? 'anthropic' : 'openai';
      const model = provider === 'anthropic' ? 'claude-3-5-haiku-20241022' : 'gpt-3.5-turbo';
      ...
      const messages = [
        {
          role: 'system',
          content: 'You are an expert in qualitative data analysis. Create exactly 3 distinct personas for coding research documents based on the provided text content.'
        },
        {
          role: 'user',
          content: `Based on this text, create exactly 3 distinct personas that would approach coding this content differently. Each persona should have: id, name, description, perspective, and keywords.

Text: ${combinedText}

Return as JSON array with exactly 3 personas.`
        }
      ];

      const response = await APIService.callAPI(provider, model, apiKey, messages, 1000, true); // skipModelUpdate = true
  ...
  // Fallback personas when AI generation fails
  function generateFallbackPersonas() {
    return [
      {
        id: 'analytical_persona',
        name: 'Analytischer Codierer',
        description: 'Fokussiert auf strukturierte, systematische Analyse',
        perspective: 'Sucht nach Mustern und logischen Verbindungen',
        keywords: ['struktur', 'analyse', 'system', 'methode', 'prozess']
      },
      {
        id: 'interpretive_persona',
        name: 'Interpretativer Codierer',
        description: 'Betont Bedeutung und Kontext',
        perspective: 'Interessiert an tieferen Bedeutungen und Zusammenhängen',
        keywords: ['bedeutung', 'kontext', 'interpretation', 'verstehen', 'sinn']
      },
      {
        id: 'critical_persona',
        name: 'Kritischer Codierer',
        description: 'Hinterfragt Annahmen und sucht nach Widersprüchen',
        perspective: 'Analysiert kritisch und sucht alternative Sichtweisen',
        keywords: ['kritik', 'frage', 'widerspruch', 'alternative', 'herausforderung']
      }
    ];
  }

  // Calculate how well a text segment matches a persona
  function calculatePersonaMatch(segment, persona) {
    const segmentLower = segment.toLowerCase();
    const keywordMatches = persona.keywords.filter(keyword =>
      segmentLower.includes(keyword.toLowerCase())
    ).length;

    const perspectiveMatch = persona.perspective &&
      segmentLower.includes(persona.perspective.toLowerCase().split(' ')[0]) ? 0.3 : 0;

    const keywordScore = Math.min(keywordMatches / persona.keywords.length, 1) * 0.7;

    return keywordScore + perspectiveMatch;
  }

  // Generate explanation for a persona's coding decision
  function generatePersonaExplanation(segment, persona, category, matchScore) {
    const matchedKeywords = persona.keywords.filter(keyword =>
      segment.toLowerCase().includes(keyword.toLowerCase())
    );

    if (matchedKeywords.length > 0) {
      return `${persona.name} kodiert diesen Abschnitt als "${category?.name}" aufgrund der Schlagwörter: ${matchedKeywords.join(', ')}. Übereinstimmung: ${(matchScore * 100).toFixed(0)}%`;
    } else {
      return `${persona.name} kodiert basierend auf der Perspektive: "${persona.perspective}". Übereinstimmung: ${(matchScore * 100).toFixed(0)}%`;
    }
  }

  // Generate combined explanation from multiple personas
  function generateCombinedExplanation(personaCodings, personaExplanations, consensus) {
    const activePersonas = Object.keys(personaCodings).filter(id => personaCodings[id]);

    if (activePersonas.length === 0) {
      return "Keine Persona hat diesen Abschnitt kodiert.";
    }

    if (consensus) {
      const explanations = activePersonas.map(id => personaExplanations[id]).filter(Boolean);
      return `Konsens erreicht! ${explanations.join(' ')}`;
    } else {
      return `Kein Konsens: ${activePersonas.length} von 3 Personas haben kodiert.`;
    }
  }
  ...
  async function generateCodings(project) {
    ...
    // Generate 3 personas from document content using AI
    let documentPersonas = [];
    try {
      documentPersonas = await generateDocumentPersonas(project.documents);
    } catch (error) {
      console.warn('AI persona generation failed, using fallback personas:', error);
      documentPersonas = generateFallbackPersonas();
    }

    project.documents.forEach(doc => {
      // Split document into segments
      const segments = (doc.content || doc.text || '').split(/\n\n+/)
      ...
      segments.forEach((segment, segIndex) => {
        ...
        // Use document-based personas for codings with explanations
        const personaCodings = {};
        const personaExplanations = {};
        documentPersonas.forEach(persona => {
          const matchScore = calculatePersonaMatch(segment, persona);
          if (matchScore > 0.6) {
            personaCodings[persona.id] = category?.id;
            // Generate explanation for this persona's coding decision
            personaExplanations[persona.id] = generatePersonaExplanation(segment, persona, category, matchScore);
          } else {
            personaCodings[persona.id] = null;
          }
        });

        // Calculate consensus (need at least 2 of 3 personas to agree)
        const codingCategories = Object.values(personaCodings).filter(Boolean);
        const uniqueCategories = new Set(codingCategories);
        const consensus = uniqueCategories.size === 1 && codingCategories.length >= 2;

        // Generate combined explanation from agreeing personas
        const explanation = generateCombinedExplanation(personaCodings, personaExplanations, consensus);

        // Estimate page number (rough approximation based on segment position)
        const pageNumber = Math.max(1, Math.ceil((segIndex + 1) / 3)); // ~3 segments per page
        ...
        codings.push({
          ...
          consensus,
          explanation, // Begründung für die Kodierung
          pageNumber, // Geschätzte Seitenzahl
          timestamp: new Date().toISOString(),
          metadata: {
            wordCount: segment.split(/\s+/).length,
            position: segIndex / segments.length,
            personaExplanations // Einzelne Begründungen der Personas
          }
        });
      });
    });
      ...
      <div className="p-6 space-y-6 bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 min-h-screen">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <button
              onClick={generateScientificArticle}
              disabled={state.ui.processing || !state.project.documents.length}
              className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-yellow-600 to-orange-600 text-white rounded-xl hover:from-yellow-700 hover:to-orange-700 disabled:opacity-50 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
            >
              <Sparkles className="w-5 h-5" />
              <span className="font-semibold">Artikel generieren</span>
            </button>
