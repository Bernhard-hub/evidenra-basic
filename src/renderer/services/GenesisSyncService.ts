/**
 * GENESIS SYNC SERVICE - EVIDENRA BASIC
 * ======================================
...
class GenesisSyncService {
  ...
  private clientType: ClientType = 'basic'; // EVIDENRA Basic
  ...
  constructor() {
    ...
    // Detect client type - always 'basic' for this build
    ...
    console.log(`[GenesisSYNC] EVIDENRA Basic initialized (${this.clientId})`);
  }
  ...
  private getOrCreateClientId(): string {
    if (typeof localStorage === 'undefined') return `basic_${Date.now()}`;

    const key = 'evidenra_client_id';
    let clientId = localStorage.getItem(key);
    if (!clientId) {
      clientId = `basic_${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
